...
void input_init(struct input_state *state, Lfm *lfm) {
  state->input_watcher.data = lfm;

  ev_idle_init(&state->input_buffer_watcher, input_buffer_cb);
  state->input_buffer_watcher.data = lfm;
  // increase the priority so we handle input before redrawing
  ev_set_priority(&state->input_buffer_watcher, 1);

  f64 repeat = (f64)cfg.map_clear_delay / 1000.0;
  ev_timer_init(&state->map_clear_timer, map_clear_timer_cb, 0, repeat);
  state->map_clear_timer.data = lfm;

  repeat = (f64)cfg.map_suggestion_delay / 1000.0;
  ev_timer_init(&state->map_suggestion_timer, map_suggestion_timer_cb, 0,
                repeat);
  state->map_suggestion_timer.data = lfm;
}

void input_deinit(struct input_state *state) {
  vec_input_drop(&state->seq);
  queue_input_drop(&state->input_buffer);
}

void input_resume(struct input_state *state, struct notcurses *nc) {
  int fd = notcurses_inputready_fd(nc);
  ev_io_init(&state->input_watcher, stdin_cb, fd, EV_READ);
  ev_io_start(event_loop, &state->input_watcher);
}

void input_suspend(struct input_state *state) {
  ev_io_stop(event_loop, &state->input_watcher);
  ev_timer_stop(event_loop, &state->map_suggestion_timer);
  ev_timer_stop(event_loop, &state->map_clear_timer);
}
...
static void stdin_cb(EV_P_ ev_io *w, i32 revents) {
  (void)revents;
  Lfm *lfm = w->data;
  struct input_state *state = &lfm->ui.input_state;
  ...
  while (notcurses_get_nblock(lfm->ui.nc, &in) != (u32)-1) {
    ...
    if (in.id == NCKEY_FOCUS) {
      ...
      if (macro_recording)
        macro_add_key(key);
      input_handle_key(state, lfm, key);
    }
  }
}

// clear the current key sequence and reset the state
static inline void input_clear(struct input_state *state, Ui *ui) {
  state->cur = NULL;
  ui_menu_hide(ui);
  if (!vec_input_is_empty(&state->seq)) {
    vec_input_clear(&state->seq);
    ui_redraw(ui, REDRAW_CMDLINE);
  }
}

void input_buffer_add(struct input_state *state, input_t in) {
  queue_input_push(&state->input_buffer, in);
  ev_idle_start(event_loop, &state->input_buffer_watcher);
}
...
static void input_buffer_cb(EV_P_ ev_idle *w, i32 revents) {
  ...
  while (!queue_input_is_empty(&state->input_buffer)) {
    input_t in = *queue_input_front(&state->input_buffer);
    queue_input_pop(&state->input_buffer);
    input_handle_key(state, lfm, in);
  }
}

static inline void handle_non_input_mode_key(struct input_state *state,
                                             Lfm *lfm, input_t in) {
  Ui *ui = &lfm->ui;
  Fm *fm = &lfm->fm;
  ...
  if (!state->cur) {
    state->cur = lfm->current_mode->maps;
    vec_input_clear(&state->seq);
    state->count = -1;
    state->accept_count = true;
  }

  // accumulate numbers to pass as a count to the next command
  if (state->accept_count && '0' <= in && in <= '9') {
    if (state->count < 0) {
      state->count = in - '0';
    } else {
      state->count = state->count * 10 + in - '0';
    }
    if (state->count > 0) {
      vec_input_push(&state->seq, in);
      ui_redraw(ui, REDRAW_CMDLINE);
    }
    return;
  }

  state->cur = trie_find_child(state->cur, in);

  // handle escape key
  if (in == NCKEY_ESC) {
    u32 mode = 0;
    if (!vec_input_is_empty(&state->seq)) {
      input_clear(state, &lfm->ui);
    } else {
  ...
  if (!state->cur) {
    ...
    usize len;
    u32 i = 0;
    c_foreach(it, vec_input, state->seq) {
    ...
    buf[i] = 0;
    log_debug("unmapped sequence: %s (last: id=%d shift=%d ctrl=%d alt=%d)",
  ...
  if (state->cur->is_leaf) {
    i32 ref = state->cur->ref;
    input_clear(state, &lfm->ui);
    lfm_lua_cb_with_count(lfm->L, ref, state->count);
    return;
  }

  // accumulate input and wait for more
  vec_input_push(&state->seq, in);
  state->accept_count = false;
  ui_redraw(ui, REDRAW_CMDLINE);

  ev_timer_again(event_loop, &state->map_clear_timer);
  ev_timer_again(event_loop, &state->map_suggestion_timer);
}

static inline void handle_input_mode_key(struct input_state *state, Lfm *lfm,
                                         input_t in) {
  Ui *ui = &lfm->ui;

  // reset the buffer/trie only if no mode map and no input map are possible
  if (!state->cur && !state->cur_input) {
    state->cur = lfm->current_mode->maps;
    vec_input_clear(&state->seq);
    state->count = -1;
    state->accept_count = true;
  ...
  if (!iswprint(in) && state->cur_input == NULL) {
    ...
    // to the command line
    state->cur_input = state->input_maps;
  }
  ...
  if (state->cur_input != NULL) {
    state->cur_input = trie_find_child(state->cur_input, in);
    if (state->cur_input) {
      // current key sequence is a prefix/full match of a mode mapping,
      // always taking precedence
      if (state->cur_input->ref) {
        i32 ref = state->cur_input->ref;
        state->cur_input = NULL;
        lfm_lua_cb_with_count(lfm->L, ref, -1);
      } else {
...
void input_handle_key(struct input_state *state, Lfm *lfm, input_t in) {
  ...
  if (lfm->current_mode->is_input) {
    handle_input_mode_key(state, lfm, in);
  } else {
    handle_non_input_mode_key(state, lfm, in);
  }
}
...
static void map_clear_timer_cb(EV_P_ ev_timer *w, i32 revents) {
  (void)revents;
  Lfm *lfm = w->data;
  input_clear(&lfm->ui.input_state, &lfm->ui);
...
static void map_suggestion_timer_cb(EV_P_ ev_timer *w, i32 revents) {
  ...
  vec_trie maps = trie_collect_leaves(lfm->ui.input_state.cur, true);
  vec_trie_sort(&maps);
